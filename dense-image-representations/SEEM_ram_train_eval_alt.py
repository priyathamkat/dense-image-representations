...
class RamDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        expanded_embedding = torch.concatenate((embedding, 1e-6*torch.ones(55-object_num, 512)), axis=0)
        expanded_rel_target = self._get_target(55, gt_rels)

        # print(f"{idx=}, {object_num=}, {sample['feat'].shape=}, {expanded_embedding.shape=}, {expanded_rel_target.shape=}, {gt_rels.shape=}, {embed_mask.shape=}", flush=True)
        return expanded_embedding, expanded_rel_target, torch.Tensor([object_num])
...
class RamModel(nn.Module):
    ...
    def loss(self, pred, target, attention_mask, N_lst):
        loss_dict = dict()
...
class RamTrainer(object):
    ...
    def train(self):
        wandb.init(project="SEEM_RAM_Training")
        t_start = time.time()
        running_avg_loss = 0
        for epoch_idx in range(self.config.num_epoch):
            for batch_idx, batch_data in enumerate(self.dataloader):
                ...
                object_nums = batch_data[2].to(torch.int).to(self.device)
                ...
                loss_dict = self.model.loss(batch_pred, batch_target, attention_mask, object_nums)
                loss = loss_dict["loss"]
                ...
                if batch_idx % 100 == 0:
                    t_current = time.time()
                    num_finished_step = epoch_idx * len(self.dataloader) + batch_idx + 1
                    ...
                    wandb.log({
                        "epoch_idx": epoch_idx,
                        "batch_idx": batch_idx,
                        "total_batches": batch_idx + epoch_idx*len(self.dataloader),
                        "LR": self.lr_scheduler.get_last_lr()[0],
                        "Loss": loss.item(),
                        "RunningAvgLoss": running_avg_loss / num_finished_step,
                        "Recall@20": recall_20.item()
                    })
...
if __name__ == "__main__":
    ...
    config = dict(
        ...
        num_epoch=100,
        output_dir="/cmlscratch/snawathe/dense-image-representations/training_output_alt",
        load_from=None,
    )
